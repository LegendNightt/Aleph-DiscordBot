...
class applications(commands.Cog):
    ...
    async def application_handler(self, application_info, questions_info):
        ...
        channel = await self.guild.create_text_channel(name=f'{data[config["ingame_name_question"]]} application',
                                                       category=self.client.get_channel(
                                                           config['applications_category']))
        ...
        user = list(
            filter(lambda u: u.name == discord_name[0] and u.discriminator == discord_name[2], self.guild.members))
        if user:
            await message.channel.set_permissions(user[0], read_messages=True, send_messages=True, attach_files=True)

        message = await self.voting_channel.send(
            embed=discord.Embed(title=f'Vote on {data[config["ingame_name_question"]]}', color=0xADD8E6))
